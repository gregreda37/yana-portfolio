...
export default function AdminSignup() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        ...
        setError(msg);
        logger.error('auth.google_signup_failed', { code, message: err?.message });
      }
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blush-50 to-lavender-50 flex items-center justify-center px-6 py-10">
      <div className="bg-white rounded-3xl shadow-lg p-10 w-full max-w-sm">
        <div className="text-center mb-8">
          <span className="font-display text-4xl text-blush-500 font-light">Yana</span>
          <p className="font-body text-xs text-gray-400 uppercase tracking-widest mt-1">Create Account</p>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl px-4 py-3 font-body text-sm text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-60 disabled:cursor-not-allowed mb-5"
        >
          <GoogleIcon />
        ...
        <p className="font-body text-xs text-gray-400 text-center mt-6">
          ...
          <Link to="/admin/login" className="text-blush-500 hover:text-blush-600 font-semibold transition-colors">
            Sign in
          </Link>
        </p>
